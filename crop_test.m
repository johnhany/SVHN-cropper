...
for i = 1:length(digitStruct)
    im = imread(strcat('test\\', digitStruct(i).name));
    for j = 1:length(digitStruct(i).bbox)
        try
            ...
            imwrite(cropped, strcat('test_crop\\', num2str(digitStruct(i).bbox(j).label), '\\', num2str(count(digitStruct(i).bbox(j).label)), '.png'));
